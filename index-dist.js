...
function reactElementToJSXString(ReactElement) {
  ...
  var ignoreDefaultProps = !!options.ignoreDefaultProps;
  var getExpression = options.expression;
  ...
  function toJSXString(_ref) {
    ...
    var defaultProps = typeof ReactElement.type === 'function' && ReactElement.type.defaultProps;

    var out = '<' + tagName;
    var elementProps = Element.props;
    if (ignoreDefaultProps && defaultProps) {
      elementProps = (0, _lodashObjectPick2['default'])(elementProps, function (value, name) {
        return defaultProps[name] !== value;
      });
    }
    var props = formatProps(elementProps);
    var attributes = [];
    var children = _react2['default'].Children.toArray(elementProps.children).filter(onlyMeaningfulChildren);
  ...
  function getJSXAttribute(name, value) {
    if (getExpression) {
      var expression = getExpression(value, name);
